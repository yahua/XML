...
@property (nonatomic, copy) FtpOperationProgressBlock uploadProgress;
...
//                [self stopReceiveWithStatus:@"Network read error"];
            ...
            if (self.bufferOffset != self.bufferLimit) {
                NSInteger   bytesWritten;
                bytesWritten = [self.outputStream write:&buffer[self.bufferOffset] maxLength:self.bufferLimit - self.bufferOffset];
                assert(bytesWritten != 0);
                if (bytesWritten == -1) {
                    //Network write error
                } else {
